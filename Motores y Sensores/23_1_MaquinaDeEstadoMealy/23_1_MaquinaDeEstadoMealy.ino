/*23.1.-MAQUINA DE ESTADO FINITO TIPO MEALY: Las FSM (Finite State Machines) están conformadas de 
...
En la máquina de Mealy las salidas se producen en las transiciones entre estados, por lo que su 
velocidad es demasiado rápida, eso es bueno cuando la secuencia ingresada es a través de medios 
digitales, pero no cuando se ingresa manualmente.
...
int delayAntirrebotes = 200;  //Tiempo de espera que el programa deja pasar como antirrebotes = 150 ms.
...
//EJECUCIÓN DEL PROGRAMA EN UN BUCLE INFINITO: FSM tipo Mealy que identifica la secuencia 1011, la salida no se mantiene estática.
void loop() {
  ...
  //MÁQUINA DE ESTADOS TIPO MEALY: Las salidas de la FSM se declaran en sus transiciones.
  if(estadoPresente == 0){        //Estado 0 = Inicio.
    ...
    duracion, realizando una simple resta entre dos medidas de tiempo y creando una comparación con un 
    ...
    if(tiempoFSM - tiempoDeEsperaFSM > delayAntirrebotes){
      ...
      transicionEstado = digitalRead(boton);  //Lectura del boton para ver si es presionado.
      //TRANSICIONES MANUALES DEL ESTADO 0 = Inicio:
      if(transicionEstado == 1){
        //SALIDAS DEL ESTADO 0 = Inicio:
        Serial.println("Transición 0 = Inicio a Estado_1: Se ingresó el primer 1 de la secuencia 1011.");
        ...
        estadoPresente = 1; //Transición manual al Estado 1 cuando se presione el boton.
        tiempoDeEsperaFSM = millis(); //Iniciación del temporizador del Estado 1.
      } 
    }
  }else if(estadoPresente == 1){  //ESTADO 1 = Estado_1:
    ...
    if(tiempoFSM - tiempoDeEsperaFSM > intervaloDuracionEstados){
      //SALIDAS DEL ESTADO 1 = Estado_1:
      Serial.println("Transición 1 = Estado_1 a Estado_2: Se ha ingresado la parte 10 de la secuencia 1011.");
  ...
  }else if(estadoPresente == 2){  //ESTADO 2 = Estado_2:
    ...
    if(tiempoFSM - tiempoDeEsperaFSM > delayAntirrebotes){
      ...
      if(transicionEstado == 1){
        ...
        Serial.println("Transición 2 = Estado_2 a Estado_3: Se ha ingresado la parte 101 de la secuencia 1011.");
  ...
  }else if(estadoPresente == 3){  //ESTADO 3 = Estado_3:
    ...
    if(tiempoFSM - tiempoDeEsperaFSM > delayAntirrebotes){
      ...
      if(transicionEstado == 1){
        ...
        Serial.println("Transición 3 = Estado_3 a Inicio: Se ha ingresado la secuencia completa 1011.");
        ...
        estadoPresente = 0;
      }else if(tiempoFSM - tiempoDeEsperaFSM > intervaloDuracionEstados){
        //SALIDAS DEL ESTADO 4 = unoCeroUnoUno:
        Serial.println("Transición 4 = Estado_3 a Inicio: No se recibió la secuencia completa.");
        digitalWrite(ledSecuencia, LOW); //Salida del Estado 4 = Led del Pin 13 Encendido.
        //Transición manual al Estado 0 cuando se presione el botón o de forma automática cuando pase cierto tiempo.
        estadoPresente = 0;
      }
    }
